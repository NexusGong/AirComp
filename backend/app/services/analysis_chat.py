...
ANALYSIS_SYSTEM_PROMPT = """你是AirComp能耗分析助手。用户在与您对话中可能：
...
ANALYSIS_INTENT_ONLY_PROMPT = """你是AirComp能耗分析助手。仅根据用户当前消息和上下文，输出一个 JSON 对象，只包含以下键，不要 reply。
- intent: "upload_client" | "upload_supplier" | "run_energy_calculation" | "general"
...
REPLY_GEN_PROMPT = """你是AirComp能耗分析助手。根据用户消息和当前上下文，用一两句话简短回复（中文），引导下一步或回答问题。不要输出 JSON，只输出回复正文。"""
