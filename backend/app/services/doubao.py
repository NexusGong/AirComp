"""豆包大模型 API 调用（智能录入：从文本/表格解析为空压机设备记录）"""
import json
import logging
import re
...
SYSTEM_PROMPT = """你是一个空压机设备信息抽取助手。用户会粘贴一段文本或表格（可能来自 Excel、文档），其中包含一条或多条空压机设备信息。
...
SYSTEM_PROMPT_SUPPLIER = """你是一个空压机供应商设备信息抽取助手。用户会粘贴一段文本或表格（可能来自 Excel、文档），其中包含一条或多条供应商设备信息。
...
async def parse_equipment_text(text: str) -> list[dict]:
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": "请从以下文本中抽取所有空压机设备记录，只输出 JSON 数组：\n\n" + text.strip()},
        ],
        "stream": False,
        "temperature": 0.1,
    }
...
async def parse_equipment_text_supplier(text: str) -> list[dict]:
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": "请从以下文本中抽取所有空压机供应商设备记录，只输出 JSON 数组：\n\n" + text.strip()},
        ],
        "stream": False,
        "temperature": 0.1,
    }
