<template>
  <div class="page-card equipment-page-wrap" v-if="form">
    <h1 class="page-title">修改供应商设备信息</h1>
...
onMounted(async () => {
  ...
  const res = await api.get(`machines/suppliers/${id}`)
  const r = res.data
  form.value = reactive({
    name: r.name,
    no: r.no,
...
async function onSubmit() {
  ...
  loading.value = true
  try {
    await api.put(`machines/suppliers/${route.params.id}`, {
      ...form.value,
