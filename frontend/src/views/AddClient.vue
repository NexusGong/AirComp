  ...
  <div class="equipment-card">
    ...
    <div v-if="showParsedModal" class="parsed-modal-overlay" @click.self="closeParsedModal">
      <div class="parsed-modal" @click.stop>
        ...
        <p class="parsed-modal-hint">请核对并编辑下方记录，可勾选要添加的条目，点击「确定添加」写入系统。</p>
    ...
    <p v-if="!embedded" class="table-caption">录入客户原有设备信息</p>
    ...
    <template v-if="mode === 'smart'">
      <div class="smart-entry-panel">
        ...
        <div v-if="smartSubMode === 'paste'" class="smart-input-block smart-input-paste">
          <p class="smart-input-desc">从 Excel 或文档中复制表格/文本，粘贴到下方。AI 将识别并提取多条设备记录。</p>
          <textarea v-model="smartText" class="form-control smart-textarea" rows="6" placeholder="例如：&#10;公司名称  A公司  编号  1  型号  XXX  额定功率  55  气量  10.5  品牌  阿特拉斯&#10;或粘贴多行表格内容"></textarea>
...
const embedded = computed(() => route.path === '/equipment/clients')
...
const returnLink = computed(() => {
  if (route.path === '/equipment/clients') {
    return { path: '/equipment/clients', query: { view: 'list', ...(form.name ? { company: form.name } : {}) } }
  }
  if (route.query.company) return { path: '/equipment/clients', query: { view: 'list', company: route.query.company } }
  return '/data-process'
})
...
onMounted(() => {
  if (route.query.company) form.name = route.query.company
})
...
async function parseSmartWithApi() {
  ...
  parsedRecords.value = []
  selectedIndexes.value = new Set()
  console.log('[AirComp] 智能解析请求 URL:', (api.defaults.baseURL || '') + 'machines/smart-parse')
  try {
    const { data } = await api.post('machines/smart-parse', { text }, { timeout: 70000 })
    const records = data?.records || []
    if (records.length === 0) {
      authStore.setFlash('未识别到设备记录，请调整文本后重试', 'warning')
      return
    }
...
async function parseUploadedFileWithApi() {
  ...
  try {
    ...
    const records = data?.records || []
    if (records.length === 0) {
      authStore.setFlash('未识别到设备记录，请检查表头与内容', 'warning')
      return
    }
    parsedRecords.value = records
    selectedIndexes.value = new Set(records.map((_, i) => i))
...
async function handleFile(file) {
  ...
  try {
    ...
    if (uploadedRows.value.length > 0) {
      authStore.setFlash('已解析表格，点击「AI 解析并识别多条记录」识别设备', 'success')
    } else {
...
async function onSubmit() {
  loading.value = true
  try {
    await api.post('machines/clients', { ...form, is_FC: !!form.is_FC })
